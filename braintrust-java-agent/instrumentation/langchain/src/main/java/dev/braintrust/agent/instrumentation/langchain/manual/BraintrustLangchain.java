...
public final class BraintrustLangchain {
    ...
    /**
     * Wrap an already-built OpenAiChatModel by replacing its internal HTTP client with
     * a tracing wrapper.
     */
    public static OpenAiChatModel wrapChatModel(OpenTelemetry openTelemetry, OpenAiChatModel model) {
        try {
    ...
    /**
     * Wrap an already-built OpenAiStreamingChatModel by replacing its internal HTTP client with
     * a tracing wrapper.
     */
    ...
    /**
     * Wrap an already-built AiService with TracingProxy and TracingToolExecutors.
     * Called from AiServices.build() advice — the service is already built, so we
     * wrap it with tracing proxy and instrument tool executors.
     */
    @SuppressWarnings("unchecked")
    public static <T> T wrapAiService(OpenTelemetry openTelemetry, AiServices<?> aiServices, Object builtService) {
        try {
            ...
            if (context.toolService != null) {
                ...
                if (underlyingExecutor != null) {
                    ...
                    try {
                        setPrivateField(context.toolService, "executor",
