...
public class DdAgentSmokeTest {
    ...
    private static void assertOtelTraces(OpenTelemetry otel) throws Exception {
        ...
        try (var ignored = root.makeCurrent()) {
            ...
            try (var ignored2 = child.makeCurrent()) {
                ...
                assertDdLogCorrelationActive(
                        child.getSpanContext().getTraceId(), child.getSpanContext().getSpanId());
            } finally {
                child.end();
            }
        } finally {
        ...
        assertTrue(
                ddTraces.size() == numExpectedTraces, "invalid num dd traces: " + ddTraces.size());
        var ddTrace = ddTraces.get(0);
        var numExpectedSpansPerTrace = 2;
        assertTrue(
                ddTrace.size() == numExpectedSpansPerTrace,
                "invalid num dd spans: " + ddTrace.size());
        if (BT_AGENT_ENABLED) {
    ...
    private static void assertOtelLogs(OpenTelemetry otel) {
        // dd does not have an interceptor for this at the moment, so we won't do anything other
        // than just use the api
    ...
    private static void assertDdLogCorrelationInactive() {
        ...
        assertTrue(
                "0".equals(traceId), "expected traceId='0' with no active span, got: " + traceId);
    ...
    private static void assertDdLogCorrelationActive(String traceId, String spanId) {
        ...
        if (!ddSpanId.equals(Long.toUnsignedString(expectedDdSpanId))) {
            errors.add(
                    "spanId: expected=%s actual=%s (otel=%s)"
                            .formatted(Long.toUnsignedString(expectedDdSpanId), ddSpanId, spanId));
        }

        if (!errors.isEmpty()) {
            throw new RuntimeException(
                    "DD log correlation (active span):\n  " + String.join("\n  ", errors));
        }
        System.out.println(
                "[smoke-test] DD log correlation (active span): OK (ddTraceId=%s ddSpanId=%s)"
                        .formatted(ddTraceId, ddSpanId));
    }

    /**
     * Verifies DD's OTel shim is installed as the GlobalOpenTelemetry implementation. The
     * TracerProvider, MeterProvider, and ContextPropagators should all be DD shim classes.
     */
    ...
    /**
     * Asserts DD-specific propagation headers are present and contain correct values. DD's
     * AgentTextMapPropagator injects x-datadog-trace-id (decimal, lower 64 bits of OTel trace ID)
     * and x-datadog-parent-id (decimal of OTel span ID).
     */
    ...
    /**
     * Asserts DD-specific metadata on the bridged spans: - Service name matches dd.service config -
    ...
    private static void assertDdSpanMetadata(List<MutableSpan> ddTrace) {
        ...
        if (childLocalRoot == null) {
            ...
            errors.add(
                    "child localRootSpan != root (localRoot resourceName='%s')"
                            .formatted(childLocalRoot.getResourceName()));
        }
        ...
        if (rootLocalRoot == null) {
            ...
            errors.add(
                    "root localRootSpan != self (localRoot resourceName='%s')"
                            .formatted(rootLocalRoot.getResourceName()));
        }
    ...
    /**
     * Verifies that the DD bridge converted DD spans to OTel SpanData and that the bridged spans
     * match the DD trace (same names, timing, error status).
     */
    private static void assertBridgedSpans(
            List<MutableSpan> ddTrace, int numExpectedTraces, int numExpectedSpans) {
        var braintrustSpans = DDBridge.getBridgedSpans();
        assertTrue(
                braintrustSpans.size() == numExpectedTraces,
                "bridged trace count: expected=%d actual=%d (keys=%s)"
                        .formatted(
                                numExpectedTraces,
                                braintrustSpans.size(),
                                braintrustSpans.keySet()));

        var braintrustTrace = braintrustSpans.values().iterator().next();
        assertTrue(
                braintrustTrace.size() == numExpectedSpans,
                "bridged span count: expected=%d actual=%d"
                        .formatted(numExpectedSpans, braintrustTrace.size()));
        ...
        for (var braintrustSpan : braintrustTrace) {
            ...
            assertNotNull(
                    matchingDd,
                    "no DD span matches bridged span '%s'".formatted(braintrustSpan.getName()));
            assertBridgedSpanMatchesDd(braintrustSpan, matchingDd);
        }

        System.out.println(
                "[smoke-test] Bridged spans (DD→OTel conversion): OK (%d spans)"
                        .formatted(braintrustTrace.size()));
    }
    ...
    private static void assertBridgedSpanMatchesDd(SpanData bridged, MutableSpan dd) {
        ...
        if (bridgedStartNanos != ddStartNanos) {
            errors.add(
                    "startTime(nanos): bridged=%d dd=%d"
                            .formatted(bridgedStartNanos, ddStartNanos));
        }
        ...
        if (Math.abs(bridgedDurationNanos - ddDurationNanos) > 1000) {
            errors.add(
                    "duration(nanos): bridged=%d dd=%d"
                            .formatted(bridgedDurationNanos, ddDurationNanos));
        }
    ...
    private static void assertEquals(Collection<SpanData> otelTrace, List<MutableSpan> ddTrace) {
        assertTrue(
                otelTrace.size() == ddTrace.size(),
                "trace span count mismatch %s - %s".formatted(otelTrace.size(), ddTrace.size()));
    ...
    private static void assertEquals(SpanData otelSpan, MutableSpan ddSpan) {
        ...
        if (!otelSpan.getName().contentEquals(ddSpan.getResourceName())) {
            errors.add(
                    "name: otel=%s dd.resourceName=%s"
                            .formatted(otelSpan.getName(), ddSpan.getResourceName()));
        }
        ...
        if (otelStartMicros != ddStartMicros) {
            errors.add(
                    "startTime(micros): otel=%d dd=%d".formatted(otelStartMicros, ddStartMicros));
        }

        long otelDurationMicros =
                (otelSpan.getEndEpochNanos() - otelSpan.getStartEpochNanos()) / 1000;
        long ddDurationMicros = ddSpan.getDurationNano() / 1000;
        if (Math.abs(otelDurationMicros - ddDurationMicros) > 1) {
            errors.add(
                    "duration(micros): otel=%d dd=%d"
                            .formatted(otelDurationMicros, ddDurationMicros));
        }

        var ddTags = ddSpan.getTags();
        otelSpan.getAttributes()
                .forEach(
                        (key, value) -> {
                            Object ddValue = ddTags.get(key.getKey());
                            if (ddValue == null) {
                                errors.add(
                                        "missing dd tag for otel attribute: %s=%s"
                                                .formatted(key.getKey(), value));
                            } else if (!ddValue.toString().equals(value.toString())) {
                                errors.add(
                                        "tag mismatch for %s: otel=%s dd=%s"
                                                .formatted(key.getKey(), value, ddValue));
                            }
                        });

        boolean otelError =
                otelSpan.getStatus().getStatusCode() == io.opentelemetry.api.trace.StatusCode.ERROR;
