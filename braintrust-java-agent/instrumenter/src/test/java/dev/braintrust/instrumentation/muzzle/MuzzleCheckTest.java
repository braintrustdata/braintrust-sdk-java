...
class MuzzleCheckTest {
    ...
    void failsWhenReferenceMissing() {
        Reference ref =
                new Reference.Builder("com/example/DoesNotExist").withSource("Test", 1).build();
    ...
    void cachesResultPerClassLoader() {
        ...
        Reference ref =
                new Reference.Builder("java/util/HashMap")
                        .withSource("Test", 1)
                        .withFlag(Reference.EXPECTS_PUBLIC)
                        .build();
    ...
    /**
     * When a classloader doesn't have a required class, muzzle should reject it. A minimal
     * URLClassLoader with no URLs won't have app classes.
     */
    @Test
    void rejectsClassLoaderMissingRequiredClass() {
        // Reference a class from our test code that won't be on an empty classloader
        Reference ref =
                new Reference.Builder("dev/braintrust/agent/muzzle/MuzzleCheckTest")
                        .withSource("Test", 1)
                        .withFlag(Reference.EXPECTS_PUBLIC)
                        .build();
