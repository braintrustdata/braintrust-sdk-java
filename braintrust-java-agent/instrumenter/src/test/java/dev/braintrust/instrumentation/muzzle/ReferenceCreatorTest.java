...
class ReferenceCreatorTest {
    ...
    /**
     * Scan our own TestAdvice class (which calls ArrayList.add and references HashMap) and verify
     * that external references are extracted.
     */
    @Test
    void extractsMethodReferenceFromAdvice() {
        ...
        assertTrue(
                refs.containsKey("dev.braintrust.instrumentation.openai.FakeLibraryClass"),
                "Should find reference to FakeLibraryClass; got keys: " + refs.keySet());

        Reference ref = refs.get("dev.braintrust.instrumentation.openai.FakeLibraryClass");
        assertNotNull(ref);
        ...
        for (Reference.Method m : ref.methods) {
            if (m.name.equals("doWork")) {
                ...
                assertTrue(
                        (m.flags & Reference.EXPECTS_NON_STATIC) != 0,
        ...
        for (Reference.Field f : ref.fields) {
            if (f.name.equals("VERSION")) {
                ...
                assertTrue(
                        (f.flags & Reference.EXPECTS_STATIC) != 0,
    ...
    void ignoresByteBuddyAnnotations() {
        ...
        for (String key : refs.keySet()) {
            assertFalse(
                    key.startsWith("net.bytebuddy."), "Should not contain ByteBuddy type: " + key);
        }
    }
    ...
    void followsBfsIntoHelperClasses() {
        ...
        Map<String, Reference> refs =
                ReferenceCreator.createReferencesFrom(
                        "dev.braintrust.instrumentation.openai.FakeAdvice",
                        getClass().getClassLoader(),
                        Set.of("dev.braintrust.instrumentation.openai.FakeHelper"));

        Reference libRef = refs.get("dev.braintrust.instrumentation.openai.FakeLibraryClass");
        assertNotNull(
                libRef, "BFS should find FakeLibraryClass referenced transitively via FakeHelper");
    ...
    void extractsClassFlagsFromMethodInsn() {
        ...
        assertTrue(
                (ref.flags & Reference.EXPECTS_NON_PRIVATE) != 0,
    ...
    void extractsReturnAndParameterTypeReferences() {
        ...
        assertTrue(
                refs.containsKey("dev.braintrust.instrumentation.openai.FakeResult"),
    ...
    void emptyAdviceProducesNoExternalReferences() {
        Map<String, Reference> refs =
                ReferenceCreator.createReferencesFrom(
                        "dev.braintrust.instrumentation.openai.EmptyAdvice",
                        getClass().getClassLoader());

        // EmptyAdvice doesn't reference any external classes
        assertTrue(
                refs.isEmpty(),
                "Empty advice should produce no external references, got: " + refs.keySet());
    }
}
