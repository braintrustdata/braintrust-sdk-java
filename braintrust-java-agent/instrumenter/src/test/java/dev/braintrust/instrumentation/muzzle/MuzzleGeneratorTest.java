...
class MuzzleGeneratorTest {
    ...
    /**
     * Verify that the generated $Muzzle bytecode produces a valid ReferenceMatcher whose references
     * match what ReferenceCreator would extract at runtime.
     */
    @Test
    void generatedMuzzleMatchesRuntimeReferences() throws Exception {
        ...
        Set<String> runtimeClassNames =
                Arrays.stream(runtimeRefs).map(r -> r.className).collect(Collectors.toSet());
        Set<String> generatedClassNames =
                Arrays.stream(generatedRefs).map(r -> r.className).collect(Collectors.toSet());
        assertEquals(
                runtimeClassNames,
                generatedClassNames,
        ...
        Map<String, Reference> runtimeMap =
                Arrays.stream(runtimeRefs).collect(Collectors.toMap(r -> r.className, r -> r));
        Map<String, Reference> generatedMap =
                Arrays.stream(generatedRefs).collect(Collectors.toMap(r -> r.className, r -> r));
        ...
        for (String className : runtimeClassNames) {
            ...
            assertEquals(runtime.flags, generated.flags, "Flags mismatch for " + className);
            assertEquals(
                    runtime.superName, generated.superName, "SuperName mismatch for " + className);
            assertArrayEquals(
                    runtime.interfaces,
                    generated.interfaces,
                    "Interfaces mismatch for " + className);
            assertEquals(
                    runtime.fields.length,
                    generated.fields.length,
                    "Field count mismatch for " + className);
            assertEquals(
                    runtime.methods.length,
                    generated.methods.length,
                    "Method count mismatch for " + className);

            // Check each field
            for (int i = 0; i < runtime.fields.length; i++) {
                assertEquals(
                        runtime.fields[i].name,
                        generated.fields[i].name,
                        "Field name mismatch at index " + i + " for " + className);
                assertEquals(
                        runtime.fields[i].fieldType,
                        generated.fields[i].fieldType,
                        "Field type mismatch for " + className + "#" + runtime.fields[i].name);
                assertEquals(
                        runtime.fields[i].flags,
                        generated.fields[i].flags,
            ...
            for (int i = 0; i < runtime.methods.length; i++) {
                assertEquals(
                        runtime.methods[i].name,
                        generated.methods[i].name,
                        "Method name mismatch at index " + i + " for " + className);
                assertEquals(
                        runtime.methods[i].methodType,
                        generated.methods[i].methodType,
                        "Method type mismatch for " + className + "#" + runtime.methods[i].name);
                assertEquals(
                        runtime.methods[i].flags,
                        generated.methods[i].flags,
    ...
    void generatedMatcherPassesOnCurrentClassLoader() throws Exception {
        ...
        assertTrue(matcher.matches(cl), "Generated references should match on current classloader");
    }
    ...
    void helperClassesAreFilteredOut() throws Exception {
        ...
        Set<String> refClassNames =
                Arrays.stream(refs).map(r -> r.className).collect(Collectors.toSet());
        assertFalse(
                refClassNames.contains("dev.braintrust.instrumentation.openai.FakeHelper"),
                "Helper classes should be filtered out of references");

        // But FakeLibraryClass (not a helper) should still be present
        assertTrue(
                refClassNames.contains("dev.braintrust.instrumentation.openai.FakeLibraryClass"),
                    ...
                    new TypeInstrumentation() {
                        ...
                        public void transform(TypeTransformer transformer) {
                            transformer.applyAdviceToMethod(
                                    named("doSomething"),
                                    "dev.braintrust.instrumentation.openai.FakeAdvice");
                        }
                    });
        }
    }
    ...
    public static class FakeMuzzleModuleWithHelper extends InstrumentationModule {
        ...
        public List<String> getHelperClassNames() {
            return List.of("dev.braintrust.instrumentation.openai.FakeHelper");
        }
