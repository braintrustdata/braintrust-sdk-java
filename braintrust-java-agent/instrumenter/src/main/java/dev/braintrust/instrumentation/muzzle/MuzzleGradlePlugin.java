...
/**
 * ByteBuddy build-time {@link Plugin} that generates {@code $Muzzle} side-classes for each concrete
 * {@link InstrumentationModule} subclass found during compilation.
 *
 * <p>The plugin matches concrete subclasses of {@code InstrumentationModule}, instantiates them
 * reflectively, scans their advice bytecode via {@link ReferenceCreator}, and writes a {@code
 * $Muzzle.class} file to the output directory containing a pre-computed {@link ReferenceMatcher}.
 *
    ...
    public DynamicType.Builder<?> apply(
        ...
        try {
            ...
            throw new RuntimeException(
                    "Failed to instantiate " + typeDescription.getName() + " for muzzle generation",
                    e);
        }

        MuzzleGenerator.generate(module, targetDir, Thread.currentThread().getContextClassLoader());
