...
/**
 * Matches a set of {@link Reference}s against a classloader's classpath using ByteBuddy's {@link
 * TypePool} (which parses .class bytes without loading classes into the JVM).
 */
    ...
    private static boolean checkMatch(
        ...
        try {
            ...
            if (!resolution.isResolved()) {
                mismatches.add(
                        new Reference.Mismatch.MissingClass(
                                reference.sources, reference.className));
                return false;
            }
    ...
    private static void findMethodsForType(
        ...
        if (!methodsToFind.isEmpty()) {
            for (MethodDescription.InDefinedShape methodDesc :
                ...
                MethodKey key =
                        new MethodKey(methodDesc.getInternalName(), methodDesc.getDescriptor());
