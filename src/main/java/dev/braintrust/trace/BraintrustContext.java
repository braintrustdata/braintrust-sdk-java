 ...
 * gRPC services), parent information is automatically propagated via OpenTelemetry Baggage when
 * propagators are configured. See {@link BraintrustTracing} for setup instructions.
 */
@Slf4j
public final class BraintrustContext {
     ...
     *
     * <p>This method stores the parent in both the OpenTelemetry Context (for same-process access)
     * and in Baggage (for distributed tracing across process boundaries). Baggage propagates
     * automatically through W3C headers when propagators are configured.
     */
    public static Context ofExperiment(@Nonnull String experimentId, @Nonnull Span span) {
        ...
        // Also store in baggage for distributed tracing
     ...
     */
    public static Optional<String> getParentFromBaggage(@Nonnull Context ctx) {
        try {
    ...
    static ParsedParent parseParent(@Nonnull String parentStr) {
        String[] parts = parentStr.split(":", 2);
        if (parts.length != 2) {
