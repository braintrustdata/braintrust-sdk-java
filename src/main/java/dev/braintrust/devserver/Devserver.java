    ...
    private void handleStreamingEval(HttpExchange exchange, RemoteEval eval, EvalRequest request)
        ...
        try (OutputStream os = exchange.getResponseBody()) {
            try {
                ...
                if (false) {
                    parentSpec = SpanParentUtil.parseParent(request.getParent());
                }
                else {
                    ...
                    if (request.getParent() != null && request.getParent() instanceof Map) {
                        ...
                        if (objectType != null && objectId != null) {
                            ...
                            log.info("Using parent span: {} with generation: {}", parentSpec, generation);
                        }
                    }
                }
            ...
            final java.util.concurrent.atomic.AtomicInteger execCounter = new java.util.concurrent.atomic.AtomicInteger(1); // Start at 1, increment for each span
                    ...
                    datasetCase -> {
                        ...
                        var evalSpan =
                                tracer.spanBuilder("eval")
                                        ...
                                        .setAttribute("braintrust.span_attributes", json(evalSpanAttrs))
                        ...
                        try (var rootScope = Context.current().with(evalSpan).makeCurrent()) {
                            ...
                            { // run task
                                ...
                                taskSpanAttrs.put("exec_counter", execCounter.getAndIncrement());
                                if (finalGeneration != null) {
                                ...
                                try (var unused = Context.current().with(taskSpan).makeCurrent()) {
                                    ...
                                    progressData.put("id", evalSpan.getSpanContext().getSpanId());
                                    ...
                                    /// ///////////
                                    // HARDCODED FOOD DATASET DONTMERGE
                                    // row id in the dataset
                                    origin.put("id", datasetCase.id());
                                    // origin.put("created", "2025-12-03T03:36:44.060Z");
                                    origin.put("_xact_id", datasetCase.xactId());
                                    /// //////////////
                                    progressData.put("origin", origin);
                                    ...
                                    // Send just the output value as JSON string, not wrapped in an object
                                    progressData.put("data", JSON_MAPPER.writeValueAsString(taskResult.result()));
                            ...
                            { // run scorers - ONE score span for all scorers (matches Eval.java)
                                ...
                                scoreSpanAttrs.put("exec_counter", execCounter.getAndIncrement());
                                if (finalGeneration != null) {
                                ...
                                try (var unused = Context.current().with(scoreSpan).makeCurrent()) {
                                    ...
                                    for (Object scorerObj : scorers) {
                                        ...
                                        for (Score score : scores) {
                                            ...
                                            // Python doesn't send progress events for scorers, so commenting out
                                            /*
            ...
            // Build summary with experiment info (matching Python format)
